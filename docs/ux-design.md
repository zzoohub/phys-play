...
**Coming Soon 섹션 (모바일):**
- 잠긴 공간 카드를 가로 스크롤 컴팩트 카드(높이 60pt)로 표시하여 스크롤 길이를 줄인다
- 컴팩트 카드: 아이콘 + 이름 + "Coming Soon" 태그. 탭 불가 (비활성 상태)
- 근거: Zeigarnik Effect -- 존재를 보여 호기심을 유발하되, 활성 스테이션에 대한 스크롤 접근성을 해치지 않는다
- 데스크톱(3열)에서는 풀 카드로 표시하여 시각적 매력을 유지한다
...
**Purpose:** "실험 시작"으로 시뮬레이션을 실행하고, 예측과 결과를 시각적으로 비교
**Layer:** 3D Viewport (전체 활성) + Minimal 2D Overlay
**Primary Action:** "실험 시작" 버튼 탭 -> 시뮬레이션 관찰 -> Discover로 자동 전환
...
자유 실험 챌린지에서 사용자가 직접 던질 때, 방향과 힘을 시각적으로 표현하여 입력을 인지하고 제어할 수 있게 한다. 기본 챌린지에서는 변수가 JSON 고정이므로, 발사 전 변수 값(각도, 속도)이 텍스트로 표시된다.
...
| 시각 요소 | 설명 |
|----------|------|
| 변수 라벨 | "각도: 45도, 속도: 10 m/s" -- 3D 뷰포트 상단 또는 발사대 근처에 표시 |
| 발사 방향 화살표 | 고정된 각도로 표시되는 정적 화살표. 사용자 조작 불가 |
| "실험 시작" 버튼 | Primary CTA. 탭 시 고정 변수로 시뮬레이션 시작 |
...
|------|------|
| 선택지 수 | 항상 2개 |
| 레이아웃 | 가로 배치 (모바일 포함). 대비 관계(통과/실패, 맞다/아니다)를 시각적으로 강화 |
...
| Predict (trajectory) | ON (정적, 저전력, 카메라 비활성) | ON (궤적 그리기 UI) | Minimal (뒤로가기, 진행 번호) |
| Predict (binary/pattern) | ON (정적, 저전력, 카메라 활성) | ON (선택 UI) | Minimal (뒤로가기, 진행 번호) |
...
| 예측 입력 (binary) | 큰 버튼, 가로 배치 (각 45%, 높이 56pt+) | 큰 버튼, 가로 배치 |
...
| Hub 스테이션 카드 | 1열 세로 스택 (활성) + 가로 스크롤 컴팩트 (Coming Soon) | 3열 그리드 (활성 + Coming Soon 모두 풀 카드) |
...
| 카메라 조작: trajectory 예측에서 비활성, binary/pattern 예측에서 활성 | trajectory는 드로잉과 제스처 충돌. binary/pattern은 2D 오버레이 입력이므로 충돌 없고, 3D 세팅 관찰이 예측 정확도를 높임 | Cognitive Load + Fitts's Law |
...
| 투사체 기본 챌린지는 "실험 시작" 버튼 (자유 던지기 아님) | 예측 조건과 실험 조건이 동일해야 비교가 유효. 자유 던지기는 자유 실험 챌린지에서만 | Cognitive Load + P2 (예측이 먼저) |
| Coming Soon 카드를 모바일에서 가로 스크롤 컴팩트 카드로 | 스크롤 길이 축소. 존재감은 유지하되 활성 스테이션 접근성 확보 | Zeigarnik Effect + Fitts's Law |
...
| Predict(trajectory)에서 카메라 조작 | 드로잉 제스처와 충돌. binary/pattern에서는 충돌 없으므로 활성 |
