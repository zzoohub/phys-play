...
프론트엔드의 주요 모듈 경계 ([client-structure.md](./client-structure.md) 참조):
...
**hud/ — In-scene HUD [Phase 1]**
HUD 패널 (슬라이더, 토글, 에너지 바). Canvas 위에 오버레이되는 mesh + HTML 적응형 컨트롤.

**shared/ui/ — DOM overlay [Phase 1]**
Canvas 바깥 DOM UI. 설정 모달, 로딩 화면, 네비게이션, 디자인 시스템.
