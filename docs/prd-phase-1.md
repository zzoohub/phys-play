...
- **코어 루프 검증이 최우선이다.** 예측->실험->발견 루프가 실제로 학습 동기를 만드는지 확인하지 않으면 어떤 확장도 무의미하다. 역학 실험실의 3개 스테이션(투사체, 에너지, 파동)은 4가지 예측 유형(trajectory, binary, pattern, placement)을 모두 커버하여 루프의 범용성을 검증한다.
...
2. 예측 유형에 따른 입력 인터페이스 활성화:
   ...
   - **pattern:** HUD에 3~4개 패턴 이미지 선택지. 각 패턴은 간단한 시각적 미리보기로 표시
   - **placement:** 3D 뷰포트 안에서 마커/오브젝트를 직접 배치. 드래그로 위치 설정, 스냅/가이드 표시. 배치 완료 버튼으로 확정. 예측 위치는 반투명 고스트로 표시되고, 시뮬레이션 후 정답 위치와 오버레이 비교
...
2. God Hand 인터랙션 (패턴 기반, 챌린지별 상세는 `challenges.md` 참조):
   - **던지기/발사:** 클릭 드래그로 오브젝트를 잡아 당겼다 놓기. 드래그 방향/강도가 발사 각도/속도에 매핑 (투사체, 에너지 일부)
   - **조립/배치:** 드래그로 오브젝트를 3D 공간에 배치. 스냅 포인트 근처에서 자석처럼 흡착 (파동 안티소스, 에너지 트랙 빌딩)
   - **당기기/밀기:** 드래그로 오브젝트 변형 또는 이동. 실시간 시각 피드백 (진자 당기기, 피스톤 압축)
   - **설치/제거:** 클릭으로 장치 토글, 드래그로 위치 조정 (검출기, 촉매)
3. 시뮬레이션 실행 중:
   - 물리 엔진(Rapier 또는 커스텀)이 실시간 계산
   - 3D 렌더링으로 결과를 실시간 표시
   - **예측(반투명)과 실제 결과가 동시에 렌더링** (trajectory: 궤적 오버레이, placement: 예측 위치 고스트 vs 정답 위치)
...
→ 챌린지 시나리오: [challenges.md](./contents/engines/projectile/challenges.md) (10개)
→ 개념·변수: [concepts.md](./contents/engines/projectile/concepts.md)
→ Discover 소재: [discover.md](./contents/engines/projectile/discover.md)

**엔진 구현 요구사항:**

| ID | 요구사항 | 우선순위 |
|----|---------|---------|
| PRJ-001 | 투사체 엔진 핵심 물리 시뮬레이션: concepts.md의 엔진 변수 전체 지원 (gravity, angle[vec2], launchSpeed, mass, drag, spinRate/spinAxis, gravitySources 등) | P0 |
| PRJ-002 | trajectory 예측 입력: 3D 뷰포트 위 포인트 기반 궤적 그리기 (최소 3포인트) | P0 |
| PRJ-003 | placement 예측 입력: 3D 공간에 마커/오브젝트 배치 (타깃 위치, 장애물 배치 등) | P0 |
| PRJ-004 | 예측(반투명)과 실제 결과(실선)의 오버레이 렌더링 (trajectory/placement 공통) | P0 |
| PRJ-005 | God Hand 던지기: 클릭 드래그 방향/강도 → 발사 각도/속도 매핑 | P0 |
| PRJ-006 | 환경 프리셋 전환: earth/moon/mars/jupiter/space/station별 시각적 배경 변화 | P1 |
| PRJ-007 | 다중 중력원(gravitySources): N-body 시뮬레이션 (중력 슬링샷, 시공간 구슬치기) | P1 |
| PRJ-008 | 마그누스 효과: spinRate/spinAxis에 따른 3D 힘 방향 계산 및 궤적 편향 | P1 |
| PRJ-009 | 모바일 터치: 스와이프 제스처 던지기, 핀치 줌 뷰 조절 | P1 |

**예측 정답 판정:**

| 예측 유형 | 판정 방법 | tolerance 기본값 |
|----------|----------|----------------|
| trajectory | DTW 또는 영역 기반 오차 측정 | 0.15 (15%) |
| placement | 유클리드 거리 또는 방향 오차 | 챌린지별 `predict.tolerance` |
| binary | 정답/오답 | — |
| pattern | 선택지 매칭 | — |

### 4.2 에너지 스테이션

→ 챌린지 시나리오: [challenges.md](./contents/engines/collision-energy/challenges.md) (8개)
→ 개념·변수: [concepts.md](./contents/engines/collision-energy/concepts.md)
→ Discover 소재: [discover.md](./contents/engines/collision-energy/discover.md)

**엔진 구현 요구사항:**

| ID | 요구사항 | 우선순위 |
|----|---------|---------|
| ENG-001 | 충돌/에너지 엔진 핵심 물리: concepts.md의 엔진 변수 전체 지원 (height, friction, mass[], velocity[vec3], elasticity, trackPieces, structure 등) | P0 |
| ENG-002 | 에너지 바 시각화: 위치에너지(파랑)/운동에너지(빨강)/열에너지(주황) 실시간 바 그래프 | P0 |
| ENG-003 | 3D 충돌 시뮬레이션: 탄성/비탄성 충돌 + 3D 운동량 보존 + 변형 시각화 | P0 |
| ENG-004 | 진자 시뮬레이션: 단일/다중 진자 (pendulumWave, 푸코 진자 포함) | P0 |
| ENG-005 | 마블런 트랙 빌더: trackPieces 기반 3D 트랙 조립 + 물리 시뮬레이션 | P0 |
| ENG-006 | God Hand 조작: 오브젝트 던지기, 트랙 조립, 진자 당기기, 보호 구조물 설계 | P0 |
| ENG-007 | 마찰/열 시각화: 열 에너지 파티클 + "사라진" 에너지의 시각적 추적 | P1 |
| ENG-008 | 뉴턴 제3법칙 시각화: 양쪽 물체의 힘/가속도를 동시 표시 (트럭 vs 경차) | P1 |

### 4.3 파동 스테이션

→ 챌린지 시나리오: [challenges.md](./contents/engines/wave/challenges.md) (8개)
→ 개념·변수: [concepts.md](./contents/engines/wave/concepts.md)
→ Discover 소재: [discover.md](./contents/engines/wave/discover.md)

**엔진 구현 요구사항:**

| ID | 요구사항 | 우선순위 |
|----|---------|---------|
| WAV-001 | 파동 엔진 핵심 물리: concepts.md의 엔진 변수 전체 지원 (waveType, frequency, roomDimensions, wallMaterial, antiSources, resonantObject 등) | P0 |
| WAV-002 | 3D 음파 시뮬레이션: 방 크기/형태/재질에 따른 3D 정상파·간섭 패턴 | P0 |
| WAV-003 | placement 예측 입력: 안티 스피커 배치, 타격점 선택 등 3D 공간 배치 | P0 |
| WAV-004 | 도플러/마하콘 시뮬레이션: 3D 경로 이동 음원의 파장 변화 + 마하콘 렌더링 | P0 |
| WAV-005 | God Hand 조작: 스피커 배치, 물체 이동, 악기 타격 | P0 |
| WAV-006 | 공명 시뮬레이션: 진동수 매칭 시 resonantObject 변형 모드 시각화 | P1 |
| WAV-007 | 지진파 시뮬레이션: 종파/횡파의 3D 전파 차이 + 지구 내부 레이어 | P1 |
| WAV-008 | 파동 시각화: 컬러맵(보강=밝음, 상쇄=어두움) + 3D 볼류메트릭 표시 | P0 |
...
```jsonc
{
  ...
  "predict": {
    ...
    "tolerance": "number",           // trajectory/placement 유형: 오차 허용 범위 (0~1, 기본 0.15)
    "options": ["string"],           // binary/pattern 유형 전용: 선택지 배열
    "placementRules": {              // placement 유형 전용 (선택)
      "snapToGrid": "boolean",      // 그리드 스냅 여부
      "maxObjects": "number",        // 배치 가능 최대 오브젝트 수
      "showGuides": "boolean"        // 가이드라인 표시 여부
    },
    "drawMode": "string"             // trajectory 유형 전용 (선택): "2d-overlay" | "3d-freehand"
  },
  "discover": {
...
**Step 4 — 반복감 방지:**
- 직전 챌린지와 동일한 God Hand 패턴(던지기/배치/조립 등) 및 핵심 변수 세트의 챌린지는 배제
- 같은 예측 유형이 3회 연속되면 다른 예측 유형의 챌린지를 우선 선택 (4종: trajectory, binary, pattern, placement)
...
| 중앙~하단 | 예측 입력 영역 | trajectory: 3D 뷰포트 위 드로잉 레이어 / binary: 2개 버튼 / pattern: 3~4개 이미지 카드 / placement: 3D 뷰포트 내 오브젝트 배치 모드 |
...
interface ChallengeResult {
  ...
  predictType: "trajectory" | "binary" | "pattern" | "placement";
...
3. 4가지 예측 유형(trajectory, binary, pattern, placement) 모두 동작
...
| XR 모드 구현 | Must 3D/XR 콘텐츠 설계 + 아키텍처 대비는 Phase 1에서, 실제 XR 구현은 Phase 2+ | Phase 2+ |
